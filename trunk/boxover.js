...
The following table lists settings for BoxOver. Parameter	Possible values	Default 	Description
...
function defHdrStyle() {
	...
	dvHdr.style.border='1px solid #A2A2A2';
	dvHdr.style.padding='3';
	dvHdr.style.fontSize='11';
	dvHdr.style.color='#6A6A6A';
...
function defBdyStyle() {
	dvBdy.style.borderBottom='1px solid #A2A2A2';
	dvBdy.style.borderLeft='1px solid #A2A2A2';
	dvBdy.style.borderRight='1px solid #A2A2A2';
	dvBdy.style.width='150px';
	dvBdy.style.fontFamily='tahoma';
	dvBdy.style.fontSize='11';
	dvBdy.style.padding='3';
	dvBdy.style.color='#6C6C6C';
...
function scanBO(curNode) {
	  if (checkElemBO(curNode.id)) {
         curNode.boHDR=getParam('header',curNode.id);
         curNode.boBDY=getParam('body',curNode.id);
			curNode.boCSSBDY=getParam('cssbody',curNode.id);			
			curNode.boCSSHDR=getParam('cssheader',curNode.id);
			curNode.IEbugfix=(getParam('hideselects',curNode.id)=='on')?true:false;
			curNode.fixX=parseInt(getParam('fixedrelx',curNode.id));
			curNode.fixY=parseInt(getParam('fixedrely',curNode.id));
			curNode.absX=parseInt(getParam('fixedabsx',curNode.id));
			curNode.absY=parseInt(getParam('fixedabsy',curNode.id));
			curNode.offY=(getParam('offsety',curNode.id)!='')?parseInt(getParam('offsety',curNode.id)):10;
			curNode.offX=(getParam('offsetx',curNode.id)!='')?parseInt(getParam('offsetx',curNode.id)):10;
			curNode.fade=(getParam('fade',curNode.id)=='on')?true:false;
			curNode.fadespeed=(getParam('fadespeed',curNode.id)!='')?getParam('fadespeed',curNode.id):0.04;
			curNode.delay=(getParam('delay',curNode.id)!='')?parseInt(getParam('delay',curNode.id)):0;
			if (getParam('requireclick',curNode.id)=='on') {
				curNode.requireclick=true;
				document.all?curNode.attachEvent('onclick',showHideBox):curNode.addEventListener('click',showHideBox,false);
				document.all?curNode.attachEvent('onmouseover',hideBox):curNode.addEventListener('mouseover',hideBox,false);
			}
			else {// Note : if requireclick is on the stop clicks are ignored   			
   			if (getParam('doubleclickstop',curNode.id)!='off') {
   				document.all?curNode.attachEvent('ondblclick',pauseBox):curNode.addEventListener('dblclick',pauseBox,false);
   			}	
   			if (getParam('singleclickstop',curNode.id)=='on') {
   				document.all?curNode.attachEvent('onclick',pauseBox):curNode.addEventListener('click',pauseBox,false);
   			}
   		}
			curNode.windowLock=getParam('windowlock',curNode.id).toLowerCase()=='off'?false:true;
			curNode.id='';
			curNode.hasbox=1;
	   }
	   else
